...
function Notifier(tls_options, /* Bool */ development) {
  ...
  function feedback(callback) {
    ...
    fb.on('close', function () { 
      vlog("+++ Feedback connection closed!");
      if (callback) callback();
    });
  ...
  function notify(/* Notification */ notification, /* Function(error) */ callback) {
  ...
  function cleanupAfterErrorOnNotification (uid) {    
    for (var identifier in notifications) {
      if (identifier > uid) { // Resend the other notifications
        // console.log('### Repushing into pending', identifier, uid);
  ...
  push = Push(tls_options, push_options).on('authorized', function () {
    ...
    vlog("*** Sent notification", n.identifier, n.device.token);
    ...
    // Callback with error
    if (notifications[uid] && notifications[uid].callback) notifications[uid].callback.call(notifications[uid], error);
